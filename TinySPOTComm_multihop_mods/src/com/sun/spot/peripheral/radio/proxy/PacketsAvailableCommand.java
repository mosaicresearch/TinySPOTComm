 ...
 */
class PacketsAvailableCommand extends RadioRequestEnvelope {
	private ConnectionID cid;
	
	public PacketsAvailableCommand(ConnectionID cid) {
		this.cid = cid;
	}

	public ReplyEnvelope execute(IRadioProtocolManager radioProtocolManager) {
		boolean result = radioProtocolManager.packetsAvailable(cid);
		return new BooleanReplyEnvelope(getUid(), result);
	}
}
