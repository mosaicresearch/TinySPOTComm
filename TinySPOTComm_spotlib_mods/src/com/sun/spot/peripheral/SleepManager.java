...
class SleepManager extends Thread implements ISleepManager {
	...
	private static final long INITIAL_TEAR_DOWN_TIME = 400;
	private static final long INITIAL_SET_UP_TIME = 400;
	...
	private void deepSleep(long targetMillisToWakeUp) {
		...
		try {
			...
			if (tracing) Utils.log("[SleepManager] " + e.getMessage());
	...
	private void primDeepSleep(long targetMillisToWakeUp) {
		...
		if (tracing) Utils.log("Returned from " + sleepTypeDescription + " at " + setUpStartTime);
