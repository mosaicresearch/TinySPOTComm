...
/**
 * Methods to read/write chips supporting the Common Flash Interface.
 */
class CFI_Flash extends Resource implements IFlashMemoryDevice {
	
    public static final int S29PL032J   = 1;
	public static final int M29W640GB70 = 2;

    public static final int BIG_SECTOR_SIZE = 0x10000;

	private static final int S29PL032J_BIG_SECTORS   = 62;  // Number of 64k sectors
	private static final int S29PL032J_SIZE          = 4 * 1024 * 1024;  // size in bytes

    private static final int M29W640GB70_BIG_SECTORS = 127;
	private static final int M29W640GB70_SIZE        = 8 * 1024 * 1024;  // size in bytes
	...
	private static final int NUMBER_OF_SMALL_SECTORS_AT_START = 8;

	private static final int SMALL_SECTOR_SIZE = 0x2000;
	private static final int ADDRESS_OF_FIRST_BIG_SECTOR = FLASH_BASE_ADDR + (NUMBER_OF_SMALL_SECTORS_AT_START * SMALL_SECTOR_SIZE);
    ...
    /*
     * Layout of flash for M29W640GB70
		// 8k
		FLASH_BASE_ADDR + 0x000000, //00
		FLASH_BASE_ADDR + 0x002000, //01
		FLASH_BASE_ADDR + 0x004000, //02
		...
		FLASH_BASE_ADDR + 0x030000, //10
        ...
		FLASH_BASE_ADDR + 0x7E0000, //133
		FLASH_BASE_ADDR + 0x7F0000, //134
     */

	private int flashType;
	private int numberOfBigSectors;
	private int numberOfSmallSectorsAtEnd;
	private int addressOfFirstSmallSectorAtTop;
    private int size;


    private static int getNumberOfBigSectors(int rev) {
        return (rev < 8) ? S29PL032J_BIG_SECTORS : M29W640GB70_BIG_SECTORS;
    }

	/**
	 * Return the sector address in the area for the CFI flash device used in the
     * specified hardware revision SPOTs.
     *
	 * @param sectorNumber
     * @param rev SPOT hardware revision
	 * @return the address of the sector
	 */
	public static int getSectorAddress(int sectorNumber, int rev) {
		if (sectorNumber >= (NUMBER_OF_SMALL_SECTORS_AT_START + getNumberOfBigSectors(rev))) {
			throw new IllegalArgumentException("Sector number " + sectorNumber + " is too large");
		} else {
			return primGetSectorAddress(sectorNumber, getNumberOfBigSectors(rev));
		}
	}
	
	/**
	 * Return the total size in bytes of the CFI flash device used in the
     * specified hardware revision SPOTs.
     *
     * @param rev SPOT hardware revision
	 * @return the flash size
	 */
	public static int getSize(int rev) {
		return (rev < 8) ? S29PL032J_SIZE : M29W640GB70_SIZE;
	}


	/**
	 * Return the number of the last large sector for the CFI flash device used in the
     * specified hardware revision SPOTs.
     *
     * @param rev SPOT hardware revision
	 * @return the sector number
	 */
	public static int getLastLargeSector(int rev) {
		return getNumberOfBigSectors(rev) + NUMBER_OF_SMALL_SECTORS_AT_START - 1;
	}

	private static int primGetSectorAddress(int sectorNumber, int numberOfBigSectors) {
		if (sectorNumber < NUMBER_OF_SMALL_SECTORS_AT_START) {
			return ConfigPage.FLASH_BASE_ADDRESS + (sectorNumber * SMALL_SECTOR_SIZE);
		} else if (sectorNumber < NUMBER_OF_SMALL_SECTORS_AT_START + numberOfBigSectors){
			return ConfigPage.FLASH_BASE_ADDRESS + (NUMBER_OF_SMALL_SECTORS_AT_START * SMALL_SECTOR_SIZE) + (sectorNumber-NUMBER_OF_SMALL_SECTORS_AT_START)*BIG_SECTOR_SIZE;
		} else {
			return ConfigPage.FLASH_BASE_ADDRESS +
						(NUMBER_OF_SMALL_SECTORS_AT_START * SMALL_SECTOR_SIZE) +
						(numberOfBigSectors * BIG_SECTOR_SIZE) +
						(sectorNumber - NUMBER_OF_SMALL_SECTORS_AT_START - numberOfBigSectors)*SMALL_SECTOR_SIZE;
		}
	}
	...
	CFI_Flash(int rev) {
        if (rev < 8) {
            flashType = S29PL032J;
            numberOfBigSectors = S29PL032J_BIG_SECTORS;
            numberOfSmallSectorsAtEnd = 8;
            size = S29PL032J_SIZE;
        } else {
            flashType = M29W640GB70;
            numberOfBigSectors = M29W640GB70_BIG_SECTORS;
            numberOfSmallSectorsAtEnd = 0;
            size = M29W640GB70_SIZE;
        }
	...
	public void eraseChip() {
		throw new SpotFatalException("The eraseChip operation is not supported.");
	}
	...
	public int getSectorSize(int sectorNum) {
		...
		if (sectorNum < NUMBER_OF_SMALL_SECTORS_AT_START || sectorNum >= (NUMBER_OF_SMALL_SECTORS_AT_START + numberOfBigSectors)) {
...
//        System.err.println("[CFI_Flash] writing to " + Integer.toHexString(address) + " : " + numOfBytes + " : " + offset);
		validateAddress(address);
		...
		if (! writtenOk)
			throw new SpotFatalException("Failed to write flash at address " + Integer.toHexString(address));
	}
	...
	public int getNumberOfSectors() {
		return numberOfBigSectors + NUMBER_OF_SMALL_SECTORS_AT_START + numberOfSmallSectorsAtEnd;
	}
	...
	public int getSectorContainingAddress(int addr) {
		...
		if (addr < ADDRESS_OF_FIRST_BIG_SECTOR) {
			...
			return ((addr - ADDRESS_OF_FIRST_BIG_SECTOR) / BIG_SECTOR_SIZE) + NUMBER_OF_SMALL_SECTORS_AT_START;
		} else {
			return ((addr - addressOfFirstSmallSectorAtTop) / SMALL_SECTOR_SIZE) + NUMBER_OF_SMALL_SECTORS_AT_START + numberOfBigSectors;
		}
	}


	public int getLastSectorAvailableToJava() {
		return NUMBER_OF_SMALL_SECTORS_AT_START + numberOfBigSectors - 1;
	}
	...
	private boolean isInRange(int address) {
		return !(address < FLASH_BASE_ADDR || address >= FLASH_BASE_ADDR + size);
	}

	private void validateSectorNumber(int sectorNum) {
		if (sectorNum < 0 || sectorNum > (numberOfBigSectors + NUMBER_OF_SMALL_SECTORS_AT_START + numberOfSmallSectorsAtEnd)) {
